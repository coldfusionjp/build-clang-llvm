...
WORKDIR /root
ARG LLVM_GIT_TAG="llvmorg-8.0.1-rc3"
...
RUN cmake3 -G "Unix Makefiles" \
		...
		-DCMAKE_EXE_LINKER_FLAGS="-Wl,--plugin-opt=O2 -Wl,--gc-sections -Wl,--as-needed -Wl,--strip-all" \
		-DCMAKE_SHARED_LINKER_FLAGS="-Wl,--plugin-opt=O2 -Wl,--gc-sections -Wl,--as-needed -Wl,--strip-all" \
		-DCMAKE_MODULE_LINKER_FLAGS="-Wl,--plugin-opt=O2 -Wl,--gc-sections -Wl,--as-needed -Wl,--strip-all" \
		-DCLANG_DEFAULT_CXX_STDLIB="libc++" \
		-DCLANG_DEFAULT_LINKER="lld" \
		-DLLVM_ENABLE_PROJECTS="clang;libcxx;libcxxabi;lld" \
		-DLLVM_ENABLE_LLD="ON" \
